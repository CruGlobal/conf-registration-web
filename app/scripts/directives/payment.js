    ...
    controller: function (
        ...
        if (angular.isUndefined(currentPayment.paymentType)) {
          ...
          switch (currentPayment.paymentType) {
            ...
            case 'TRANSFER':
              ...
              switch (currentPayment.transfer.accountType) {
                case 'STAFF':
                ...
                case 'NON_US_STAFF':
                  if (
                    ...
                    !currentPayment.transfer.operatingUnit ||
                    !currentPayment.transfer.department ||
                    !currentPayment.transfer.accountNumber
                  ) {
                    paymentErrors.push(
            ...
            case 'CHECK':
              ...
              break;
            case 'GIFT_CARD':
              if (!currentPayment.giftCard.number) {
                paymentErrors.push(
                  gettextCatalog.getString('Please enter a gift card number.'),
                );
              } else {
                const cardLength = currentPayment.giftCard.number.length;
                if (cardLength < 10) {
                  paymentErrors.push(
                    gettextCatalog.getString(
                      'Gift card number must be at least 10 characters.',
                    ),
                  );
                } else if (cardLength > 12) {
                  paymentErrors.push(
                    gettextCatalog.getString(
                      'Gift card number must be no more than 12 characters.',
                    ),
                  );
                }
              }
      ...
      $scope.accountTypeChanged = () => {
        ...
        [
          ...
          $scope.currentPayment.transfer.accountType === 'NON_US_STAFF'
            ? ['INTLM', 'GENL']
            : ['', ''];
      };
    },
