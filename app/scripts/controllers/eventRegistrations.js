...
angular
  ...
  .controller('eventRegistrationsCtrl', function(
    $rootScope,
    $scope,
    $uibModal,
    modalMessage,
    ...
    $scope.toggleBuiltInColumn = function(columnName) {
      $scope.builtInColumnsVisible[columnName] = !$scope.builtInColumnsVisible[
        columnName
      ];
      localStorage.setItem(
        'builtInColumnsVisibleStorage',
    ...
    $scope.viewFormStatus = registrant => {
      ...
      $uibModal.open(formStatusModalOptions).result.then(updatedRegistrant => {
