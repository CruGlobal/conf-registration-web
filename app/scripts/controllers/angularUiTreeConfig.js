angular
  ...
  .controller(
    'AngularUiTreeConfig',
    function ($scope, modalMessage, $sanitize, gettextCatalog) {
      $scope.toolbarTreeConfig = {
        ...
        beforeDrop: function (event) {
          ...
          if (event.dest.nodesScope.$nodeScope) {
            //prevents error from dropping on source tree
      ...
      $scope.pageTreeConfig = {
        accept: function (sourceNodeScope, destNodesScope) {
          var sourceType =
            sourceNodeScope.$modelValue.pageId ||
            sourceNodeScope.$modelValue.defaultTitle
              ? 'block'
              : 'page';
        ...
        beforeDrop: function (event) {
          ...
          var sourcePageIndex = _.findIndex(conference.registrationPages, {
            id: sourcePageId,
          });

          var destPageId = event.dest.nodesScope.$nodeScope.$modelValue.id;
          var destPageIndex = _.findIndex(conference.registrationPages, {
            id: destPageId,
          });
          ...
          angular.forEach(block.rules, function (rule) {
            ...
            if (
              parentBlockLocation.page > destPageIndex ||
              (parentBlockLocation.page === destPageIndex &&
                parentBlockLocation.block >= event.dest.index)
            ) {
              rulesViolated.push(
                '<strong>' +
                  $sanitize(block.title) +
                  '</strong> ' +
                  gettextCatalog.getString('must be below') +
                  ' <strong>' +
                  $sanitize(parentBlockLocation.title) +
                  '</strong>.',
              );
            }
          });
          ...
          var allRules = _.flatten(
            _.map(
              _.flatten(_.map(conference.registrationPages, 'blocks')),
              'rules',
            ),
          );
          var rulesLinkedToBlock = _.filter(allRules, {
          ...
          angular.forEach(rulesLinkedToBlock, function (rule) {
            ...
            if (
              ...
              (childBlockLocation.page === destPageIndex &&
                childBlockLocation.block < event.dest.index) ||
              (childBlockLocation.page === destPageIndex &&
              ...
              rulesViolated.push(
                '<strong>' +
                  $sanitize(childBlockLocation.title) +
                  '</strong> ' +
                  gettextCatalog.getString('must be below') +
                  ' <strong>' +
                  $sanitize(block.title) +
          ...
          if (rulesViolated.length) {
            modalMessage.error({
              ...
              message:
                '<p><strong>' +
                gettextCatalog.getString('Rule violations:') +
                '</strong></p><ul><li>' +
                rulesViolated.join('</li><li>') +
                '</li></ul>',
            });
            return false; //cancel regular drop action
          }
