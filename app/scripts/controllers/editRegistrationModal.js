angular
  ...
  .controller('editRegistrationModalCtrl', function(
    $scope,
    $uibModalInstance,
    modalMessage,
    $http,
      ...
      if (setRegistrationAsCompleted) {
        modalMessage
          .confirm({
            ...
            question:
              'Are you sure you want to mark this registration as completed?',
          })
          .then(
            function() {
        ...
        originalRegistrantObject.registrantTypeId !==
        $scope.adminEditRegistrant.registrantTypeId
      ) {
        ...
        angular.forEach($scope.adminEditRegistrant.answers, function(a) {
          if (
            !angular.equals(
              a,
              _.find(originalRegistrantObject.answers, { id: a.id }),
            )
          ) {
    ...
    function saveAllAnswers() {
      $http
        .put(
          'registrations/' + originalRegistrantObject.registrationId,
          $scope.registration,
        )
    ...
    function getRegistrantAndClose() {
      $http
        .get('registrations/' + originalRegistrantObject.registrationId)
